...
| `bold_sections`         | Use legacy bold section headings   | `False`       |
...
Existing schema version `1` files do not need to add `bold_sections`. When it is
omitted, generated sections use H3 Markdown headings. Set it to `true` to use
the legacy bold-text format.
